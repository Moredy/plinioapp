...
export default function Mensagens({ navigation }) {
    ...
    useEffect(() => {
        ...
        getequipemedicaInfoFromDatabase();
        getAllMsgDeUid();
    ...
    const convertUidEquipeMedicaParaNome = async (myMsgs) => {
        ...
        for(let i =0 ; i< myMsgsNew.length ; i++) {
            await firebase.database().ref('/equipemedica/' +  myMsgsNew[i].de + "/name/").once('value').then(async (snapshot) => {
    ...
    const [equipemedica, setEquipeMedica] = useState();
    ...
    const getequipemedicaInfoFromDatabase = () => {
        firebase.database().ref('/equipemedica/').on('value', (snapshot) => {
            if (snapshot.val()) {

                setEquipeMedica(snapshot.val())
    ...
    const getAllMsgDeUid = () => {
        ...
        firebase.database().ref('/mensagens/').on('value', (snapshot) => {
            ...
            convertUidEquipeMedicaParaNome(myMsg);
        });
        ...
        <Container>
            <Content style={{ marginTop: 50 }}>
                ...
                <Form>
                    ...
                    >
                        <Picker.Item label="Selecione a equipe medica" value="" />

                        {equipemedica ? Object.keys(equipemedica).map((prop, key) => {
                            return (
                                <Picker.Item label={equipemedica[prop].name} value={prop} />
                            );
